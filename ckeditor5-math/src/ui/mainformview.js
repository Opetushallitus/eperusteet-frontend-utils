...
export default class MainFormView extends View {
  ...
  constructor(locale) {
    ...
    // Preview element
    this.renderView = new LabelView(locale);
    this.renderView.extendTemplate({
      attributes: {
        class: [
          'ck-math-preview'
        ]
    ...
    // Submit button
    this.saveButton = this._createButton(t('Insert'), checkIcon, 'ck-button-save', null);
  ...
  _createInputView() {
    ...
    inputView.on('input', e => {
      this.texEq = e.source.element.value;
      // x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}
      Katex.render(this.texEq, this.renderView.element);
    });

    return mathInput;
  }

  _createButton(labelKey, icon, className, eventName) {
    const button = new ButtonView(this.locale);

    button.set({ labelKey, icon, tooltip: true });
